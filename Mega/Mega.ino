...
#define LOOP_OPTIMIZE  				// loop cycle time optimization - print on TFT only when data are changed
#define NODELAY_SERIALCOM_NANO		// Uncomment if using serial communication to Nano without delay() function
#define NODELAY_SERIALCOM_NODEMCU	// Uncomment if using serial communication to NodeMCU without delay() function
#define NODELAY_SERIALCOM_TFT		// Uncomment if using serial communication to TFT without delay() function
...
#if defined(NODELAY_SERIALCOM_NANO) or defined(NODELAY_SERIALCOM_NODEMCU) or defined(NODELAY_SERIALCOM_TFT)
#include "SerialCom_non_blocking.h"
#endif
#if defined(NODELAY_SERIALCOM_NANO)
SerialCom SerialCom1 (Serial1, 40);
#endif
#if defined(NODELAY_SERIALCOM_NODEMCU)
SerialCom SerialCom2 (Serial2, 80);
#endif
#if defined(NODELAY_SERIALCOM_TFT)
...
#define Microswitch_1  48               // Define Pin 47 on the MEGA to detect the microswitch  (Bumper RH or Lift Top End Stop)
...
****************************************************************************************************/
  ...
  // Safety Tilt Feature
  int  Angle_Sensor_Enabled           = 1;      //EEPROM       // Prvents Mower from climibing steep hills
  int  Tip_Over_Sensor_Enabled        = 1;      //EEPROM       // Turns mower off if turned over
  ...
  bool Wheels_Activate           = 0;       //EEPROM //RVES add // Wheels ON/OFF - for testing
...
#endif
#if defined(NODELAY_SERIALCOM_NODEMCU)
  if (WIFI_Enabled == true) Serial2.begin(57600);					// If WIFI is on open Serial port 2 for the NodeMCU communication
  if (WIFI_Enabled == true) SerialCom2.begin ();
#else
