...
#define SerialMEGA Serial1  // Change to Serial port you use for communication with Mega, origin value = Serial1
...
Adafruit_GFX_Button UP1_BTN, UP2_BTN, UP3_BTN, UP4_BTN, UP5_BTN, UP6_BTN,
                    ...
                    Done_Compass_btn, Save_Wheel_Amp_btn, Done_GPS_btn;
...
bool refreshTFT = true;     // LOOP_OPTIMIZE bit
...
int	   Compass_Heading_Degrees_Last; //LOOP_OPTIMIZE added
float  Heading;
...
#else
    SerialMEGA.begin(9600);          // 9600 Start the serial for transmitting the commands to the main MEGA on the mower
#endif
...
//delay(4500);  // NODELAY_SERIALCOM dissabled
Get_Initial_Values();
...
if ((Robot_Status_Value == 1) || (Robot_Status_Value == 2)) {
    ...
    while (Menu_Complete_Main == false) {
              ...
              refreshTFT = false; // LOOP_OPTIMIZE bit
...
if (Robot_Status_Value == 6) {
    Serial.println(F("Mower is in Manual"));
    //while (Robot_Status_Value == 6) {
    		// TODO not ready in Mega
          //}
    //tft.fillScreen(BLACK);
    }
...
//Mower is in an undefined status
//*************************************************************************************************
if (Robot_Status_Value == 0 || Robot_Status_Value == 3 /*|| Robot_Status_Value == 8*/) {
    Serial.println(F("Mower is in undefined status"));
    while (Robot_Status_Value == 0 || Robot_Status_Value == 3 /*|| Robot_Status_Value == 8*/) {
