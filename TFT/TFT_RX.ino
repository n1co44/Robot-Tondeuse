...
void Receive_7_Data_Sets() {
  ...
  while (SerialMEGA.available() > 0) {  //RVES change-different Serial used
...
void Get_Initial_Values() {
    ...
    if (Debug_RX_Transfer == 1) {              // Display the received values on the TFT Screen if required.        
      ...
      //const String day = dayAsString(Time_Day);
      char buf[4];
	  ...
	  Serial.print(Time_Year);
	  Serial.print(F("-"));
	  if (Time_Month < 10) Serial.print("0");
	  Serial.print(Time_Month);
	  Serial.print(F("-"));
	  if (Time_Date < 10) Serial.print("0");
	  Serial.print(Time_Date);
	  Serial.print(F(" "));
      Serial.print(Time_Hour);
      Serial.print(F(":"));
...
void Receive_Aerator_Running_Data() {
  ...
  while (SerialMEGA.available() > 0) {
    ...
    && recieved != '\f' && recieved != '\g' && recieved != '\h' && recieved != '\i' ) { 
      ...
      Drill_Status = Serial1_RX_Value.toInt();                                 
      Serial1_RX_Value = ""; // changed to string
      } 
      else if (recieved == '\s') {
      Robot_Status_Value = Serial1_RX_Value.toInt();                                 
...
String Serial2_RX_Value  = "";                                              
  while (Serial2.available() > 0) {
    ...
    char recieved = SerialMEGA.read();  //TODO: really combi Serial1 a nd Serial2 ???
    if ( recieved != '\a' && recieved != '\b' && recieved != '\c') {   
