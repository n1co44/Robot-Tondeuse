...
void Print_Wheel_Motors_Menu() {  
  ...
  tft.fillScreen(BLACK);
    ...
    Wheels_Activate   = Data6; // RVES added
    Data7 = 0;
    Clear_Data_Streams();
    ...
    if (Debug_RX_Transfer == 1) {              // Display the received values on the TFT Screen if required.        
      ...
      tft.print(F("Wheels ON = ")); // RVES added
      tft.println(Wheels_Activate); // RVES added
      tft.println(F(" ")); 
      delay(TFT_Check);
      tft.fillScreen(BLACK);     
      }
...
//Wheels ON/OFF Button  //  RVES added

    if (Robot_Type == 1) {
        if (Wheels_Activate == 1) OPTION1_BTN.initButton(&tft, 230, 200, 160, 40, WHITE, GREEN, BLACK, "WHEELS ON", 2);
        if (Wheels_Activate == 0) OPTION1_BTN.initButton(&tft, 230, 200, 160, 40, WHITE, RED, WHITE, "WHEELS OFF", 2);
        }
...
//If the MAG Slow Button is pressed
 ...
 // Wheels ON/ OFF - RVES added
 if (OPTION1_BTN.justPressed() )  {

         bool Changed = 0;

         if ((Wheels_Activate == 1) && (Changed == 0))  {
        	 Wheels_Activate  = 0;
           Changed = 1;
           if (Robot_Type == 1 ) OPTION1_BTN.initButton(&tft, 230, 200, 160, 40,  WHITE, RED, WHITE, "WHEELS OFF", 2);
           if (Robot_Type == 2 ) OPTION1_BTN.initButton(&tft, 230, 200, 160, 40,  WHITE, RED, WHITE, "WHEELS OFF", 2);
           OPTION1_BTN.drawButton(false);
           }

         if ((Wheels_Activate  == 0) && (Changed == 0)) {
        	 Wheels_Activate  = 1;
           Changed = 1;
           if (Robot_Type == 1 )  OPTION1_BTN.initButton(&tft, 230, 200, 160, 40,  RED, GREEN, BLACK, "WHEELS ON", 2);
           if (Robot_Type == 2 )  OPTION1_BTN.initButton(&tft, 230, 200, 160, 40,  RED, GREEN, BLACK, "WHEELS ON", 2);
           OPTION1_BTN.drawButton(false);
         }
    }
...
void Sense_Button_Press_Wheel_Motor() {
    ...
    OPTION1_BTN.press    (down && OPTION1_BTN.contains(pixel_x, pixel_y)); // Wheels ON/OFF - RVES added
    ...
    if (OPTION1_BTN.justReleased()) OPTION1_BTN.drawButton(); // Wheels ON/OFF - RVES added
