...
void TelnetSpy::setDebugOutput(bool en) {
	...
	if (debugOutput) {
		...
		os_install_putc1( TelnetSpy_putc);  // Set system printing (os_printf) to TelnetSpy // fix base on: https://github.com/yasheena/telnetspy/issues/13
		...
		if (actualObject == this) {
			...
			os_install_putc1( TelnetSpy_ignore_putc); // Ignore system printing  // fix base on: https://github.com/yasheena/telnetspy/issues/13
#else // ESP32
			// ToDo: How can be done this for ESP32 ?
#endif
			actualObject = NULL;
